...
export default class App extends React.Component {
  ...
  addTodo(newTodo) {
    ...
    */
    fetch('/api/todos', {
      ...
      .then(response => response.json())
      .then(newTodo => {
        const todosCopy = this.state.todos.slice(0);
  ...
  toggleCompleted(todoId) {
     ...
     */
    let index = null;
    let updateIsCompleted = null;
    for (let i = 0; i < this.state.todos.length; i++) {
      if (todoId === this.state.todos[i].todoId) {
        index = i;
        updateIsCompleted = this.state.todos[index].isCompleted;
      }
    }
    const newIsCompleted = { isCompleted: !updateIsCompleted };
